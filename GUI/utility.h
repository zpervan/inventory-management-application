...
static bool LoadTextureFromFile(const char* filename, ID3D11ShaderResourceView** out_srv, int* out_width, int* out_height)
{
    ...
    window::g_pd3dDevice->CreateTexture2D(&desc, &subResource, &pTexture);
    ...
    window::g_pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc, out_srv);
    pTexture->Release();
...
#endif  // INVENTORYMANAGEMENTAPPLICATION_GUI_UTILITY_H_
